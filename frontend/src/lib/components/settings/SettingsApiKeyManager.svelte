<script lang="ts">
  ...
  interface Props {
    ...
    // Parent-owned for the same reason: a request outlives a collapse.
    busy: Record<string, boolean>;
    onchanged: () => Promise<void> | void;
  }

  let { kind, keys, placeholder, drafts, revealed, busy, onchanged }: Props = $props();
    ...
    <table class="table">
      ...
      <tbody>
        {#each keys as key (key.id)}
          <tr>
            <td>
              <button type="button" class="btn btn-ghost btn-sm" disabled={busy[kind]} onclick={() => renameKey(key)} title="Rename" aria-label="Rename {key.label}">
                {key.label}
            ...
            <td class="text-right">
              <button type="button" class="btn btn-ghost btn-sm" disabled={busy[kind]} onclick={() => (pending = { action: 'rotate', key })}>
                <Icon name="refresh" size="sm" />
                Rotate
              </button>
              <button type="button" class="btn btn-ghost btn-sm btn-danger-text" disabled={busy[kind]} onclick={() => (pending = { action: 'revoke', key })}>
...
<div class="key-new">
  ...
  />
  <button type="button" class="btn btn-primary" disabled={busy[kind]} onclick={createKey}>
