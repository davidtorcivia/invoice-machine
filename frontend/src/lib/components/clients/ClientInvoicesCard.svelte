<script lang="ts">
  ...
  let { invoices = [], onnewinvoice, onopeninvoice } = $props();
</script>

<div class="card">
  <div class="card-header">
    <h3 class="card-title">Invoices</h3>
    <button class="btn btn-secondary btn-sm" onclick={() => onnewinvoice?.()}>
      ...
      <table class="table">
        ...
        <tbody>
          {#each invoices as invoice}
            ...
            <tr onclick={() => onopeninvoice?.(invoice.id)} class="clickable-row">
  ...
  {:else}
    <div class="empty-state-small">
      ...
      <p>No invoices yet</p>
      <button class="btn btn-primary btn-sm" onclick={() => onnewinvoice?.()}>Create Invoice</button>
    </div>
  {/if}
</div>
