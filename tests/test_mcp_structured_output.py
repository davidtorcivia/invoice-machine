...
async def test_tools_publish_output_schemas(mcp_db):
    ...
    for name in [
        ...
        "list_invoice_payments", "record_invoice_payment",
...
async def test_money_survives_the_round_trip_as_exact_strings(mcp_db):
    ...
    for field in ("amount_paid", "amount_outstanding", "amount_refunded", "tax_amount"):
        assert isinstance(structured[field], str), f"{field} was coerced to {type(structured[field])}"
